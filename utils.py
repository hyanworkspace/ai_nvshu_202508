...
def make_chinese_english_pairs(chinese_list, english_list):
    """
    将中文和英文两个等长列表整理成 [{'chinese': ..., 'english': ...}, ...] 的形式
    """
    return [
        {"chinese": c, "english": e}
        for c, e in zip(chinese_list, english_list)
    ]
...
class Machine:
    ...
    def replace_with_el(self, message):
        ...
        if not all_in_el:
            possible_choices = [char for char in message if (char not in ['，', '。', '！', '？']) and (not char in self.knowledge.known_mappings.keys())]
...
def format_error_response(error_type, message, details=None, status_code=500):
    """
    格式化错误响应
...
def handle_ai_function_error(error, function_name="AI功能"):
    """
    处理AI功能相关的错误
    ...
    # 根据错误类型返回不同的响应
    if "词向量系统" in error_msg or "word_vectors" in error_msg:
        return format_error_response(
            "系统初始化错误",
            f"{function_name}初始化失败，请联系管理员检查知识库文件",
            error_msg,
            500
        )
    elif "知识库文件" in error_msg or "pickle" in error_msg.lower():
        return format_error_response(
            "知识库错误",
            "知识库文件损坏或缺失，请联系管理员重新初始化",
            error_msg,
            500
        )
    elif "诗句参数" in error_msg or "参数无效" in error_msg:
        return format_error_response(
            "参数错误",
            "提供的参数格式不正确，请检查输入数据",
            error_msg,
    ...
    elif "API" in error_msg or "网络" in error_msg:
        return format_error_response(
            "网络错误",
            "AI服务暂时不可用，请稍后重试",
            error_msg,
        ...
        return format_error_response(
            "处理失败",
            f"{function_name}处理时出现未知错误，请稍后重试",
            error_msg,
            500
        )

def validate_poem_input(poem):
    """
    验证诗句输入
    ...
    if not isinstance(poem, str):
        return False, "诗句必须是字符串类型"
    
    if len(poem.strip()) < 5:
        return False, "诗句长度不足，至少需要5个字符"
    
    # 检查是否包含中文字符
    import re
    if not re.search(r'[\u4e00-\u9fff]', poem):
        return False, "诗句必须包含中文字符"
