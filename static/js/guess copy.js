    ...
    try {
        ...
        if (replaceData.poem_in_simple_el) {
            ...
            if (charData.char_translate) {
                ...
                if (speakerTranslate) speakerTranslate.textContent = charData.char_translate;
                // if (listenerTranslate) listenerTranslate.textContent = charData.char_translate;
            }
...
function startAutomaticRevelation() {
    ...
    // // 设置初始翻译（第一个猜测的英文翻译）
    // const listenerTranslate = document.getElementById('listener-translate');
    // if (guessCharsEng.length > 0) {
    //     listenerTranslate.textContent = guessCharsEng[0];
    // }
